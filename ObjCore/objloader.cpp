			...
			if(line[0]=='v' && line[1]=='t'){
				...
				if(std::string::npos != line.find("//")){
					...
					while(getline(liness, v, ' ')){
						...
						pointList.push_back((int)::atof(f.c_str())-1);

						getline(facestring, f, '/');
						getline(facestring, f, ' ');
						normalList.push_back((int)::atof(f.c_str())-1);
					...
					while(getline(liness, v, ' ')){
						...
						while(getline(facestring, f, '/')){
							if(i==0){
								...
								texturecoordList.push_back((int)::atof(f.c_str())-1);
							}else if(i==2){
								normalList.push_back((int)::atof(f.c_str())-1);
							}
							i++;
					...
					vector<int> pointList;
					while(getline(liness, v, ' ')){
						pointList.push_back((int)::atof(v.c_str())-1);
					}
					geomesh->addFace(pointList);
